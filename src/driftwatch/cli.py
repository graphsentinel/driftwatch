"""DriftWatch CLI: `demo <scenario>`, `eval --dataset ...`, `consensus-seed`.

The five demo scenarios run the real detection core against scripted chains, so `make demo-N`
reproduces each on-stage scenario without a cluster. The orchestration side (generate/run an
AgenticArchitecture) lives in AgentGate, a separate repo — DriftWatch governs, it doesn't generate.
"""
