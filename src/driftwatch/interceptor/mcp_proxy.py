...
def make_session_interceptor_factory(template: Interceptor, task_type: str = "",
                                     agent_id: str = "") -> InterceptorFactory:
    """Factory that clones `template`'s shared parts but a fresh per-session chain.

    Shared (read-mostly): baseline store, policy knobs, emitter, contract. Per-session: the adapter,
    which holds that caller's `DecisionChain`. This is the "shared baseline + per-session chain"
    design — we do NOT clone the engine or the store per session. `agent_id` identifies which agent
    this proxy seat fronts, so the declared contract (E11) can be checked per agent.
    """
    def _make() -> Interceptor:
        return Interceptor(
            template.store,                       # shared baseline store
            KagentAdapter(task_type=task_type, agent_id=agent_id),  # per-session: fresh chain
...
def _drift_middleware_class():
    ...
    class DriftMiddleware(Middleware):
        ...
        async def on_call_tool(self, context, call_next):
            ...
            msg = context.message
            # E13 §4c — the agent passes the cross-check context as an `_meta` key in arguments
            # (FastMCP's call_tool(meta=) is client-local). Lift it out so it scores/forwards cleanly.
            meta = None
            if isinstance(msg.arguments, dict) and isinstance(msg.arguments.get("_meta"), dict):
                meta = msg.arguments.pop("_meta")     # remove before forwarding upstream
            raw = to_engine_call(msg.name, msg.arguments, meta)
            ...
            if verdict.outcome in (BLOCK, DROP):
                ...
                # E11 declared violation: no statistical decision, the reason is in signals.
                if verdict.signals.get("declared"):
                    dr = verdict.signals.get("reason", "declared violation")
                    raise ToolError(f"{verb} by DriftWatch (declared): {dr}")
                d = verdict.decision
