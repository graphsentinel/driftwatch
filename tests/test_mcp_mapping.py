...
def test_to_engine_call_carries_meta_when_present():
    from driftwatch.interceptor.mcp_mapping import to_engine_call
    out = to_engine_call("pods_list", {"namespace": "prod"},
                         {"prompt": "list pods", "tools": ["pods_list"]})
    assert out["meta"] == {"prompt": "list pods", "tools": ["pods_list"]}
    # absent meta → no "meta" key (back-compat, byte-stable for existing callers)
    assert "meta" not in to_engine_call("pods_list", {"namespace": "prod"})
